...
class PTSArticleStruct:
    def __init__(self, id, dimension, sigma, sigmaV, init="random"):
        self.id = id
...
class PTSUserStruct:
    def __init__(self, id, dimension, sigma, sigmaU, init="random"):
        self.id = id
        self.dimension = dimension
        self.sigma = sigma
        self.sigmaU = sigmaU
...
class PTSParticleStruct:
    def __init__(
        self, dimension, n, itemNum, sigma, sigmaU, sigmaV, weight, ptsb=False
    ):
        self.sigma = sigma
        self.weight = weight
        self.users = []
...
class PTSAlgorithm(BaseAlg):
    ...
    def updateParameters(self, articlePicked, click, userID):
        ...
        for d in range(self.particle_num):
            ...
            var = 1.0 / (self.particles[d].sigma ** 2) + article.V.dot(
                article.A2Inv
            ).dot(article.V)
        ...
        self.particles = [
            ...
            for i in np.random.choice(
                self.particle_num, size=self.particle_num, p=weights
            )
        ]
        for d in range(self.particle_num):
            self.particles[d].weight = 1.0 / self.particle_num
        for d in range(self.particle_num):
            self.particles[d].users[userID].updateParameters(
                self.particles[d].articles[articlePicked.id], click
            )
            self.particles[d].articles[articlePicked.id].updateParameters(
                self.particles[d].users[userID], click
            )

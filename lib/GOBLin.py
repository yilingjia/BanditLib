...
class GOBLinSharedStruct:
    def __init__(self, featureDimension, lambda_, userNum, W):
        ...
        self.STBigWInv = sqrtm(
            np.linalg.inv(np.kron(W, np.identity(n=featureDimension)))
        )
...
class GOBLinAlgorithm(CoLinUCBAlgorithm):
    def __init__(self, arg_dict):
        ...
        self.USERS = GOBLinSharedStruct(
            self.dimension, self.lambda_, self.n_users, self.W
        )

...
class CoLinUCBUserSharedStruct(object):
    def __init__(self, featureDimension, lambda_, userNum, W):
        ...
        self.alpha_t = 0.0
        self.sigma = 1.0e-200  # Used in the high probability bound, i.e, with probability at least (1 - sigma) the confidence bound. So sigma should be very small
    ...
    def updateParameters(self, articlePicked, click, userID, update="Inv"):
        ...
        if update == "Inv":
            ...
            self.AInv = self.AInv - float(
                np.dot(self.AInv, np.dot(outer, self.AInv))
            ) / (1.0 + np.dot(np.transpose(X), np.dot(self.AInv, X)))
        self.UserTheta = matrixize(
            np.dot(self.AInv, self.b), len(articlePicked.contextFeatureVector)
        )
    ...
    def getProb(self, alpha, article, userID):
        ...
        try:
            self.alpha_t = 0.01 * np.sqrt(
                np.log(np.linalg.det(self.A) / float(self.sigma * self.lambda_))
...
class CoLinUCBAlgorithm(BaseAlg):
    def __init__(self, arg_dict, update="inv"):  # n is number of users
        ...
        self.USERS = CoLinUCBUserSharedStruct(
            arg_dict["dimension"],
            arg_dict["lambda_"],
            arg_dict["n_users"],
            arg_dict["W"],

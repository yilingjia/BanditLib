...
class LinUCBUserStruct:
    ...
    def getProb(self, alpha, article_FeatureVector):
        ...
        var = np.sqrt(
            np.dot(np.dot(article_FeatureVector, self.AInv), article_FeatureVector)
        )
        pta = mean + alpha * var
        return pta
...
class Uniform_LinUCBAlgorithm(object):
    def __init__(self, dimension, alpha, lambda_, init="random"):
...
class LinUCBAlgorithm(BaseAlg):
    def __init__(self, arg_dict, init="random"):  # n is number of users
        ...
        for i in range(arg_dict["n_users"]):
            self.users.append(
                LinUCBUserStruct(arg_dict["dimension"], arg_dict["lambda_"], init)
            )
    ...
    def decide(self, pool_articles, userID, k=1):
        ...
        art_features = np.empty(
            [
                len(pool_articles),
                len(pool_articles[0].contextFeatureVector[: self.dimension]),
            ]
        )
        ...
        # VARIANCE
        var_matrix = np.sqrt(
            np.dot(np.dot(art_features, self.users[userID].AInv), art_features.T).clip(
                0
            )
        )
...
class Hybrid_LinUCBUserStruct:
    def __init__(self, featureDimension, lambda_, userFeatureList):
        ...
        for i in range(len(userFeatureList)):
            ...
            self.users.append(
                Hybrid_LinUCB_singleUserStruct(userFeatureList[i], lambda_, i)
            )

    def updateParameters(self, articlePicked_FeatureVector, click, userID):
        z = vectorize(
            np.outer(self.users[userID].userFeature, articlePicked_FeatureVector)
        )
    ...
    def getProb(self, alpha, article_FeatureVector, userID):
        ...
        temp = np.dot(
            np.dot(
                np.dot(self.A_zInv, np.transpose(self.users[userID].B)),
        ...
        -2 * np.dot(z, temp) + np.dot(
            np.dot(np.dot(x, self.users[userID].AInv), self.users[userID].B), temp
        )
...
class Hybrid_LinUCBAlgorithm(object):
    ...
    def decide(self, pool_articles, userID):
        ...
        for x in pool_articles:
            x_pta = self.USER.getProb(
                self.alpha, x.contextFeatureVector[: self.dimension], userID
            )
            if maxPTA < x_pta:
                articlePicked = x
                maxPTA = x_pta

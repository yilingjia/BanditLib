...
class LinPHEUserStruct:
    ...
    def updateParameters(self, articlePicked_featureVector, click):
        ...
        perturbed_reward = 0.1 * np.random.binomial(self.a, 0.5)
...
class LinPHEAlgorithm(BaseAlg):
    def __init__(self, arg_dict):
        ...
        for i in range(arg_dict["n_users"]):
            self.users.append(
                LinPHEUserStruct(
                    arg_dict["dimension"], arg_dict["lambda_"], arg_dict["a"]
                )
            )
    ...
    def decide(self, pool_articles, userID, k=1):
        # MEAN
        art_features = np.array(
            [
                article.contextFeatureVector[: self.dimension]

...
class CLUBUserStruct(LinUCBUserStruct):
    def __init__(self, featureDimension, lambda_, userID):
        LinUCBUserStruct.__init__(
            self, featureDimension=featureDimension, lambda_=lambda_
        )
        self.reward = 0
        self.CA = self.A
        self.Cb = self.b
    ...
    def updateParameters(self, articlePicked_FeatureVector, click, alpha_2):
        ...
        self.counter += 1
        self.CBPrime = alpha_2 * np.sqrt(
            float(1 + math.log10(1 + self.counter)) / float(1 + self.counter)
        )
    ...
    def getProb(self, alpha, article_FeatureVector, time):
        ...
        var = np.sqrt(
            np.dot(np.dot(article_FeatureVector, self.CAInv), article_FeatureVector)
        )
...
class CLUBAlgorithm(BaseAlg):
    ...
    def decide(self, pool_articles, userID, k=1):
        self.users[userID].updateParametersofClusters(
            self.clusters, userID, self.Graph, self.users
        )
        articles = []
        for i in range(k):
            maxPTA = float("-inf")
            ...
            for x in pool_articles:
                x_pta = self.users[userID].getProb(
                    self.alpha, x.contextFeatureVector[: self.dimension], self.time
                )
    ...
    def updateParameters(self, articlePicked, click, userID):
        self.users[userID].updateParameters(
            articlePicked.contextFeatureVector[: self.dimension], click, self.alpha_2
        )
    ...
    def updateGraphClusters(self, userID, binaryRatio):
        ...
        for j in range(n):
            ratio = float(
                np.linalg.norm(
                    self.users[userID].UserTheta - self.users[j].UserTheta, 2
                )
            ) / float(self.users[userID].CBPrime + self.users[j].CBPrime)

...
class ThompsonSamplingUserStruct:
    def __init__(self, featureDimension, lambda_, v_squared):
        ...
        self.theta_estimate = np.random.multivariate_normal(
            self.theta_hat, self.v_squared * np.linalg.inv(self.B)
        )
...
class ThompsonSamplingAlgorithm(BaseAlg):
    def __init__(self, arg_dict):
        ...
        v_squared = self.get_v_squared(
            arg_dict["R"], arg_dict["epsilon"], arg_dict["delta"]
        )

        for _ in range(arg_dict["n_users"]):
            self.users.append(
                ThompsonSamplingUserStruct(
                    arg_dict["dimension"], arg_dict["lambda_"], v_squared
                )
            )
    ...
    def updateParameters(self, article_picked, click, userID):
        self.users[userID].updateParameters(
            article_picked.contextFeatureVector[: self.dimension], click
        )

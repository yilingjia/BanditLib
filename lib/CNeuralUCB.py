...
class CNeuralUCBAlgorithm(BaseAlg):
    def __init__(self, arg_dict):
        ...
        self.learner = extend(MLP_model(input_dim=self.dimension, hidden_layers=self.hidden_layers).to(self.device))
    ...
    def updateParameters(self, articlePicked, click, userID):
        ...
        context_vec = np.concatenate((user_vec, article_vec.reshape(1, -1)), 1)
        self.A += self.g * self.g
        self.data.push(userID, context_vec, click)
        self.cnt = (self.cnt + 1) % self.batch
        self.learning_rate *= 0.99999977

...
class User:
    ...
    def chooseArticle(self, recommendation):
        ...
        for i in range(len(recommendation.articles)):
            reward = np.dot(
                self.estimatedTheta, recommendation.articles[i].featureVector
            )
    ...
    def initializeEstimatedTheta(self, featureDimension, lambda_, init="random"):
...
class UserManager:
    ...
    def simulateThetafromUsers(self):
        ...
        if self.UserGroups == 0:
            ...
            for i in range(self.UserGroups):
                usersids[i] = range(
                    int(self.userNum * i / self.UserGroups),
                    int((self.userNum * (i + 1)) / self.UserGroups),
                )

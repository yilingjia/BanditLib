...
from lib.CNeuralUCB import CNeuralUCBAlgorithm
from lib.CNeuralPHE import CNeuralPHEAlgorithm
...
if __name__ == "__main__":
    ...
    user["default_file"] = os.path.join(
        sim_files_folder,
        "users_"
        + str(n_users)
        + "context_"
    ...
    if ("load" in article) and article["load"]:
        articles = (
            AM.loadArticles(articles["filename"]) if "filename" in articles else AM.loadArticles(articlesFilename)
        )
    elif simulating:

...
class ArticleManager:
    def __init__(self, dimension, n_articles, ArticleGroups, FeatureFunc, argv):
        ...
        self.signature = (
            ...
            + str(self.n_articles)
            + "+AG"
            + str(self.ArticleGroups)
            + "+TF-"
            + self.FeatureFunc.__name__
        )
    ...
    def saveArticles(self, Articles, filename, force=False):
        with open(filename, "w") as f:
            for i in range(len(Articles)):
                f.write(
                    json.dumps((Articles[i].id, Articles[i].featureVector.tolist()))
                    + "\n"
                )
    ...
    def simulateArticlePool(self):
        ...
        for i in range(self.dimension):
            feature_matrix[:, i] = np.random.normal(
                0, np.sqrt(1.0 * (self.dimension - i) / self.dimension), self.n_articles
            )

...
class DatasetRewardManager:
    ...
    def runAlgorithms(self, algorithms, diffLists):
        ...
        # record the time used for each algorithm
        time_list = {}
        for alg_name, alg in algorithms.items():
            time_list[alg_name] = []
        ...
        with open(fileName, "r") as f:
            ...
            for i, line in enumerate(f, 1):
                ...
                for alg_name, alg in algorithms.items():
                    ...
                    t2 = time.time()
                    time_list[alg_name].append(t2 - t1)
                ...
                if i % 100 == 0:
                    ...
                    if i % 1000 == 0:
                        ...
                        self.write_regret_to_file(
                            filenameWriteRegret, algorithms, BatchCumlateRegret, i, RandomChoice.regret, time_list
                        )
                        # for alg_name in algorithms.keys():
                        #     print(alg_name, "time: {}".format(sum(time[alg_name])))
            self.plot_result(
                algorithms,
                BatchCumlateRegret,
                tim_,
    ...
    def set_up_regret_file(self, filenameWriteRegret, algorithms):
        ...
        with open(filenameWriteRegret, "w") as f:
            for alg in algorithms.keys():
                f.write(
                    ",".join(
                        [str(key) + ": {}".format(algorithms[alg].args[key]) for key in algorithms[alg].args.keys()]
                    )
                )
            f.write("\n")
            f.write("Time(Iteration),Random")
            f.write("," + ",".join([str(alg_name) for alg_name in algorithms.keys()]))
            f.write("," + ",".join([str(alg_name) + "_time" for alg_name in algorithms.keys()]))
            f.write("\n")

    def write_regret_to_file(self, filenameWriteRegret, algorithms, BatchCumlateRegret, iter_, randomRegret, time):
        with open(filenameWriteRegret, "a+") as f:
            f.write(str(iter_))
            f.write("," + str(randomRegret))
            f.write("," + ",".join([str(BatchCumlateRegret[alg_name][-1]) for alg_name in algorithms.keys()]))
            f.write("," + ",".join([str(sum(time[alg_name])) for alg_name in time.keys()]))
            f.write("\n")
